...
function Cards({ data, title }) {
  ...
  return (
    <div className="w-full h-full flex flex-wrap px-[3%] bg-[#1F1E24]">
      {data.map((t, i) => (
        <Link to={`/${t.media_type || title}/Detail/${t.id}`} className=" w-[26vh] mr-[3%] mb-[3%]  relative" key={i}>
        
          <img
            className="object-cover h-[40vh] shadow-[8px_17px_38px_2px_rgba(0,0,0,.5)]"
            src={t.backdrop_path || t.poster_path || t.profile_path ? `https://image.tmdb.org/t/p/original/${
